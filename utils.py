...
def img2tensor(img_arr): 
    ...
    # img_tensor = img_tensor.permute(3,0,1,2).squeeze(0)
...
def calculate_ssim(img1, img2, crop_border, input_order='HWC', test_y_channel=True):
    ...
    if np.any(np.isnan(img1)) or np.any(np.isnan(img2)):
    ...
    for i in range(img1.shape[2]):
        ...
        ssim_value = _ssim(img1[..., i], img2[..., i])

        if np.isnan(ssim_value):
            # print(f"Warning: SSIM calculation resulted in NaN for channel {i}.")
            ssims.append(0)
        else:
